...
def cronbach_alpha(x):
    ...
    """Computes Cronbach's coefficient alpha, which also known as
    ...
    Returns
    alpha       -   Float indicating the reliability.
    """
    
    # Count the number of items and participants.
    n_items, n_subjects = x.shape
    
    # Compute the variance across subjects.
    x_var = numpy.var(x, axis=1, ddof=1)
    # Compute t value.
    t = numpy.sum(x, axis=0)
    
    # Compute a value.
    alpha = n_items / (n_items-1.0) * (1.0 - numpy.sum(x_var) \
        / numpy.var(t, ddof=1))
...
if __name__ == "__main__":
    ...
    print("Cronbach's coefficient alpha is {}".format(alpha))
